...
```
irm URL | iex
  → Caixa de texto para colar as URLs
  → Seletor de pasta de destino
  → Conversão /width/600/ → /width/3840/
  → Download paralelo com até 6 conexões simultâneas
  → Relatório final
```
...
Se baixar o `.ps1` manualmente, desbloqueie o arquivo:

```powershell
Unblock-File -Path .\albummer.ps1
```
